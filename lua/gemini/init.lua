...
local DEFAULT_CONFIG = {
  ...
  colors = require("gemini.palette-dark"),
  overrides = {},
  theme = "gemini",
}
...
local function get_configs()
  ...
  if g.colors_name == "gemini-soft" then
    configs.theme = "gemini-soft"
    configs.colors = require("gemini.palette-light")
  elseif g.colors_name == "gemini" then
    configs.theme = "gemini"
    configs.colors = require("gemini.palette-dark")
  end
...
---setup gemini colorscheme
---@param configs GeminiConfig?
local function setup(configs)
  if type(configs) == "table" then
    user_configs = configs --[[@as GeminiConfig]]
  end
end

---load gemini colorscheme
---@param theme string?
local function load(theme)
  if vim.fn.has("nvim-0.7") ~= 1 then
    vim.notify("gemini.nvim: you must use neovim 0.7 or higher")
    return
  end
